...
const starts = async (zeroyt7 = new WAConnection()) => {
    ...
    fs.existsSync('./Icebot.json') && zeroyt7.loadAuthInfo('./Icebot.json')
    ...
    await zeroyt7.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./Icebot.json', JSON.stringify(zeroyt7.base64EncodedAuthInfo(), null, '\t'))

    zeroyt7.on('chat-update', async (message) => {
        require('./Icebot.js')(zeroyt7, message, _welkom)
    })
zeroyt7.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && mem.includes(zeroyt7.user.jid)) {
        zeroyt7.sendMessage(anu.jid, "Halo!.. saya ICE  Bot saya akan membatu mempermudah kehidupan..seperti membuat sticker dan lain-lain. untuk meulai silahkan ketik #menu.", "conversation")
      }
      if (!isWelkom) return
      if (anu.action == "add" && !mem.includes(zeroyt7.user.jid)) {
